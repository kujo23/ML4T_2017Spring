...
class StrategyLearner(object):
    ...
    def testPolicy(self, symbol = "AAPL", sd=dt.datetime(2009,1,1), ed=dt.datetime(2010,1,1),sv = 100000):
        ...
        df_change = pd.DataFrame(data=df_indicators.change,index=df_indicators.index)
        df_change2 = pd.DataFrame(data=df_indicators.change2,index=df_indicators.index)
        self.thres_ema = self.discretize(df_ema)
        self.thres_momentum = self.discretize(df_momentum)
        self.thres_change = self.discretize(df_change)
        self.thres_change2 = self.discretize(df_change2)

        current_hold = 3 #current_hold BUY, SELL, NOTHING - 1,2,3
        state = self.get_state(trades.index[0],current_hold,df_ema,df_momentum,df_change,df_change2)  #XYZ, XY are indicators
        ...
        for i in range(0,trades.shape[0]):
            ...
            state = self.get_state(trades.index[i],current_hold,df_ema,df_momentum,df_change,df_change2)
            if self.verbose:print 'state: ',state
    ...
    def get_state(self,idx,my_holding,df_ema,df_momentum,df_change,df_change2):
        ema = df_ema.ix[idx,0]
        momentum = df_momentum.ix[idx,0]
        change = df_change.ix[idx,0]
        change2 = df_change2.ix[idx,0]
        return self.get_discrete(self.thres_change2,change2)*10000 + self.get_discrete(self.thres_change,change)*1000 + self.get_discrete(self.thres_ema,ema)*100 + self.get_discrete(self.thres_momentum,momentum)*10 + my_holding
    ...
    def indicators(self,sd = dt.datetime(2008,1,1), ed = dt.datetime(2009,1,1),syms = ['AAPL'], n=10, gen_plot=False, verbose=False):
        ...
        df['ema']= pd.rolling_std(prices,window=7).fillna(0)

        #df['momentum'] = (df.ix[2*n:,0]/df.ix[0:-2*n:,0].values)-1.
        df['momentum']= prices.pct_change(periods=2).fillna(0)
        df['change']= prices.pct_change(periods=4).fillna(0)
        df['change2']= prices.pct_change(periods=6).fillna(0)


        return df[['ema','momentum','change','change2']]
